...
export const MAPS: GameMap[] = [
  ...
  },
  {
    ...
    // 黄风岭（土）：左下角入场（-1,9）→ 沿 c1 上行至第5行（贴栅栏走廊）→ 栅栏南侧长直横行 → c6 下行 → 右下角唐僧。
    // 两头都到底（入场与唐僧都在底边 r9），中间向上弯成凹谷；与流沙河（左中入场/右侧谷）几何不同。
    // 板内路径 16 格（原 14 格为全场最短、不好打）：谷加深一行 + 走廊加长，栅栏南侧整条 5 格直线都是火力覆盖窗口。
    path: [
      { c: -1, r: 9 }, { c: 0, r: 9 }, { c: 1, r: 9 }, // 底边左行入场（出界首点）
      { c: 1, r: 8 }, { c: 1, r: 7 }, { c: 1, r: 6 }, { c: 1, r: 5 }, // 左谷内壁上行至贴栅栏走廊
      { c: 2, r: 5 }, { c: 3, r: 5 }, { c: 4, r: 5 }, { c: 5, r: 5 }, { c: 6, r: 5 }, // 栅栏南侧长直火力走廊
      { c: 6, r: 6 }, { c: 6, r: 7 }, { c: 6, r: 8 }, // 右谷内壁下行
      { c: 7, r: 8 }, // 第8行右行一步
      { c: 7, r: 9 }, // 落到右下角唐僧
    ],
    tangseng: { c: 7, r: 9 },
