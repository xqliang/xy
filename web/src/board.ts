...
export const MAPS: GameMap[] = [
  ...
  },
  {
    ...
    // 竞品为上半场(AI，1起算)：出怪口(5,8)、唐僧(2,1)、初始槽(1,4)–(3,5)。
    // AI 路径像两个 n（双峰），中央初始块下方（第4行）联通两峰；我方为 180° 镜像（双谷 ∪∪）。
    // 下面存的是玩家路径（AI = mirrorPath 自动镜像）：入场(0,5) → 唐僧(7,8)。
    path: [
      { c: 0, r: 5 }, { c: 0, r: 6 }, { c: 0, r: 7 }, { c: 0, r: 8 }, { c: 0, r: 9 }, // 左缘下行（镜像 AI 右缘上行 = 右 n 右腿）
