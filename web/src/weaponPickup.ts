// 对局左下角神兵领取卡片：清波掉落后不自动入库，点击后才 addWeapon。
import { VIEW_H } from './render';
import { weaponById, weaponQualityColor } from './weapons';
...
export function weaponPickupHitAt(x: number, y: number, ids: string[]): string | null {
...
export function drawWeaponPickups(ctx: CanvasRenderingContext2D, ids: string[]): void {
  ...
  for (let i = 0; i < ids.length; i++) {
    ...
    ctx.fill();
    ctx.lineWidth = 2;
    ctx.strokeStyle = weaponQualityColor(1);
    ctx.stroke();
    ...
    ctx.fillText('点击领取', r.x + 56, r.y + r.h / 2 + 12);
  }
}
