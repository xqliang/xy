...
describe('英雄合成/激活时大招 CD', () => {
  it('首次激活 skillCd 从 0 开始', () => {
    const b = new Battle(1);
    b.status = 'playing';
    placeErlangPair(b);
    const g = b.activeGenerals()[0]!;
    expect(g.state.skillCd).toBe(0);
  });

  it('拆开后重新激活 skillCd 重置为 0', () => {
    const b = new Battle(1);
    ...
    expect(g2.state.skillCd).toBe(0);
  });

  it('placeFromTray 凑对激活后 skillCd 从 0 开始', () => {
    const b = new Battle(1);
    b.status = 'playing';
    const cells = b.unlockedCells();
    const a = cells[0]!;
