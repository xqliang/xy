...
## 7. 控制 / 辅助大招数值（`TUNING`，2026-08-31 更新）

| 常量 | 满5 | 满3 |
|------|-----|-----|
| 定身 `heroStunDur*` | 1.5s | 1.0s |
| 击退 `heroKnockPush*` | 1.0 格（2026-08-31 由 1.5 下调：双击退英雄每 10s 抹掉 5s 行程过强） | 1.0 格 |
| 击退伤害 `heroKnockDmgMul` | 1.35（同日 1.2→1.35：格数下调后按盘面价值公式回补，价值从控制时长转移到直伤） | 同左 |
...
**BOSS/小 Boss CC 抗性**（2026-08-31 新增 `bossCcResist = 0.5`）：妖王与小 Boss 受到的
武将定身 / 击退 / 哮天犬咬定身效果一律 ×0.5（此前高血单体在多控制武将射程内会被近乎
全锁，理论压制 ~97%）。普通妖 / 精英不受影响。玩家主动技能（如冰封 `freezeStunDur`）
暂不受该抗性影响。
