...
4. **反作弊**：服务端校验放置动作合法性 + 比对双方每 2s 上报的棋盘摘要（各端用确定性重放交叉核对）；当天有 ≥3 个**不同对手**判定该用户异常 → 通知本人并**当天禁止真人匹配**。
...
| 议题 | 决策 |
|------|------|
| 同步模型 | **输入回放（Model B）**：服务端转发双方"放置动作"，两端**固定步长确定性模拟**双方半场；对手半场是本地重放（约落后 2-4s，画面顺滑）；2s 棋盘摘要用于反作弊交叉核对 |
...
| 暂停区 | 匹配中 → **退出匹配**；对局中 → **认输** |
| 传输 | **HTTP 轮询**（2s 双向 tick + 匹配轮询），**不引入 WebSocket** |
...
玩家A 浏览器 (Web)                         玩家B 浏览器 (Web)
  ...
  │   (确定性、约落后2-4s，仅供展示与反作弊)    │
  └─ 每2s(或事件即时) POST /api/versus/tick ─┐ ┌─ 每2s POST /api/versus/tick
...
**分层保证**：`A 半场`由 A 实时权威模拟；`B 在 A 机上的重放`只吃 B 的放置动作、跑在**延迟时间线**（缓冲输入、落后实时若干秒），仅供展示与反作弊；**波次开始/胜负这类权威耦合全走服务端实时消息**，不依赖重放。→ 即便重放有残余漂移，最坏只是对手观感略偏，胜负与节奏不受影响；反作弊比对用**容差**（比对单位数/战力/唐僧血/波次等粗粒度不变量，非逐像素）。

### 3.2 传输与延迟

- 服务端标准库 `http.server`（同步、无长连接）。用 **2s 双向 tick** 承担心跳 + 放置动作批量转发 + 摘要上报 + 下发（对手动作/下一波/终局）；清波/放置/认输/阵亡等**事件即时补发 tick** 降低延迟。
- 对手重放跑在 **落后约 2-4s** 的时间线上，足以吸收 2s 转发间隔——不是逐帧锁步（帧锁步在 2s 轮询下不可行），而是"延迟确定性重放"。
- 权威耦合（下一波开始、终局）走服务端，天然容忍 1-2s 传播延迟（波次间有间隔缓冲）。
...
- **摘要**（供反作弊）：`{wave,power,kills,tangsengHP,peach,units}`，2s 一次。
...
2. **端上重放交叉核对**：每端确定性重放对手动作流，与对手 2s 自报 `digest` 做**容差比对**（单位数/战力/唐僧血/波次），偏离阈值 → 端上向服务端上报"对手摘要不符"信号（两诚实端会一致；作弊端自报与诚实端重放不符 → 暴露）。
...
| POST | `/api/versus/tick` | 2s 双向：上报动作/摘要/心跳 → 返回对手动作/下一波/终局/禁赛通知 |
...
- WebSocket/SSE/长连接（2s 轮询足够）；帧锁步（用延迟重放替代）。
- 微信端 PvP（二期）。
...
- **同步**：先清者触发的 `nextWave` 两端一致 spawn；对手重放在延迟时间线顺滑；时钟对齐误差容忍。
