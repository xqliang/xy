...
- **微信迁移的固有残余风险**：首次微信登录时用于「继承本机进度」的 local uid 是**客户端自报**的。
  谁先用某个「尚未被任何 openid 绑定」的 uid 完成微信登录，就永久把该 uid 绑到自己的 openid（受本次
  加的守卫保护：不会去抢已被别的 openid 绑定的 uid）。因此「知道某匿名 uid 的人可冒领其账号」这一匿名期
  既有暴露（§7 web 项），在微信侧会被固化成一次性的永久归属。uid 不外显（榜单脱敏、空间 ~9e15 不可枚举）
  故风险有限；服务端已对每次迁移绑定打审计日志（openid 脱敏 + adopt_uid）便于事后追查。彻底消除需迁移确认
  流程或改「新建 uid + 合并进度」，超出本期范围。
